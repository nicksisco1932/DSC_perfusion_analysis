...
in_names = ["T1w_pre.nii", "T1w_post.nii", "T2w_FLAIR.nii", "SAGE.nii.gz"]
...
def work(path):
    ...
    pool2 = mp.Pool(processes=os.cpu_count())
    ...
    for i in os.listdir(path):
        if os.path.isdir(os.path.join(path, i)):
            path_1 = os.path.join(path, i)
            print(i)
            p2 = pool2.apply_async(register_everything, args=(path_1, i,))
        else:
            pass
    pool2.close()  # do not accept any more tasks
    pool2.join()
...
def register_everything(path, filename):
    ...
    if not os.path.isfile(os.path.join(path, 'brainSAGE.nii.gz')):
        pass
    else:
        ...
        os.system('flirt -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 '
                  '-interp trilinear -in %s -out %s -ref %s -omat %s '
                  % (os.path.join(path, out_names[0] + '.gz'),
        ...
        os.system('flirt -paddingsize 0.0 -interp trilinear -in %s -out %s -ref %s -applyxfm -init %s'
                  % (os.path.join(path, in_names[0]),
                     os.path.join(path, 'T1w_pre_SAGEres.nii'),
                     os.path.join(path, out_names[3] + '.gz'),
                     os.path.join(path, 'Reg_refT1wpreSAGE_inputT1wpre.txt')))
        os.system('flirt -paddingsize 0.0 -interp trilinear -in %s -out %s -ref %s'
                  % (os.path.join(path, out_names[0] + '.gz'),
                     os.path.join(path, 'brainT1w_pre_SAGEres'),
                     os.path.join(path, out_names[3] + '.gz')))
        os.system('flirt -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 '
                  '-interp '
                  'trilinear -in %s -out %s -ref %s -omat %s'
