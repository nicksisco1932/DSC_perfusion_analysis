...
% Edited by Nicholas J. Sisco, Ph.D. 20200303; reduced number of plots.
%                                                                                                                          %
...
function AIF = AutoAIF_Brain(vol, paramsTime, flip, DCEflag, ConcFlag)
...
elseif length(size(vol)) == 5
    ...
    disp('This code only uses information for 2 echoes.'); %you'll need to code if you want to add other echoes; NS, I hate warnings; changed to disp.
    fprintf('\nThis is a multi echo dataset with %d echoes: \n', ne);
    fprintf('nx: %d ny: %d nz: %d nt: %d \n\n', nx, ny, nz, nt);
...
try
    [ss_tp, gd_tp, pk_tp, brainmask] = determineTimePoints(vol, 0, 0, TR);
catch
    disp('Determine TP failed. Using default values')
	ss_tp = 1; gd_tp = 40; pk_tp = 60; 
    brainmask = ones(nx,ny,nz); % how exactly is this a mask? It is the entire image. 
end
peak_frame = I(:,:,:,pk_tp); % only used for a figure
...
switch echoFlag
    case 1
        I = abs(vol(:,:,:,:,:));
        I_v = squeeze(reshape(I, [nx*ny*nz ne nt])); %Why is this reshaped again? There is so much redundancy
                
        %calculate delta R2 star curves
        S0_TE1 = squeeze(mean(I_v(:,1,ss_tp:gd_tp),3)); 
        S0_TE1 = repmat(S0_TE1, [1 nt]); % Repeat copies of array 
        S0_TE2 = squeeze(mean(I_v(:,2,ss_tp:gd_tp),3)); 
        S0_TE2 = repmat(S0_TE2, [1 nt]); % Repeat copies of array
         
        dR2s_TE1 = log(S0_TE1./squeeze(I_v(:,1,:)))/TE1; 
        dR2s_TE1(isinf(dR2s_TE1)) = 0; % Change infinities to 0 
        dR2s_TE1(isnan(dR2s_TE1)) = 0; % Change NaN to 0 
        ...
        switch DCEflag
            case 1 %Schmainda et al. method % No idea what reference this is. 
    ...
    otherwise %only single echo acquisition
        I_v = abs(I_v);
        S0_TE1 = squeeze(mean(I_v(:,ss_tp:gd_tp),2)); 
...
switch echoFlag
    case 1 %multiecho
        ...
        for i = 1:length(mask)
            if mask(i,1) > 0 && corr(dR2s_TE1(i,ss_tp:end)', dR2s_TE2(i,ss_tp:end)') > 0.5 % How is 0.9 defined? NS 20200326
                finalmask(i,1) = 1;
            end
        end
        ...
        loc(VXall > 100) = 0; %get rid of outliers with extreme height %NS how is extreme height defined?
        loc(loc == 0) = [];
...
for i = 1:length(loc)
    ind = loc(i);
%     figure(100);plot(I_v(loc(i),:));pause(0.05);
    bolus_pts = find(squeeze(I_v(ind,1:gd_tp+10)) < precontrast_mean(ind,:)-10*precontrast_std(ind,:));
    %bolus_pts = bolus_pts + (ss_tp-1);
    if ~isempty(bolus_pts) && length(bolus_pts)*TR < 8 %AIF shouldn't enhance longer than 8 seconds - removes enhancing voxels. This needs some explanation. This is written wrong.
        
       % This should be written as the time between the gd_tp and pk_tp.
...
% table = cat(2, sum_dR2(new_loc,:), new_loc); % it's a mistake to name a
% variable a MATLAB function. NS 20200314
% table = abs(sortrows(-table, 1)); % it's a mistake to name a variable a
% MATLAB function. NS 20200314
tab = cat(2, sum_dR2(new_loc,:), new_loc); 
tab = abs(sortrows(-tab, 1)); %why?
switch echoFlag
    case 1
        final_loc = tab(1:min(10,size(tab,1)),2); %take top 10 -- EDIT AMS20160216 - if <10 are found, take min of (10,#found)
        if size(tab,1)<10 % ADDED AMS20160216 - notify user of # of voxels found
            fprintf('\nFound %d potential AIF voxels, keeping all voxels.\n', size(tab,1));
            warning('Found less than 10 potential AIF voxels, proceeding with AIF calculation');
        else
            fprintf('\nFound %d potential AIF voxels, keeping top 10.\n', size(tab,1));
        end
    case 0
        final_loc = tab(1:min(20,size(tab,1)),2); %take top 10 -- EDIT AMS20160216 - if <10 are found, take min of (10,#found)
        if size(tab,1)<20 % ADDED AMS20160216 - notify user of # of voxels found
            fprintf('\nFound %d potential AIF voxels, keeping all voxels.\n', size(tab,1));
            warning('Found less than 20 potential AIF voxels, proceeding with AIF calculation');
        else
            fprintf('\nFound %d potential AIF voxels, keeping top 20.\n', size(tab,1));
        end
end

    
%This mask isn't really used.
...
switch echoFlag
    case 1
        h = figure; hold on;
        title('AIF \DeltaR_2^{*}', 'FontSize', 14);
        ...
        legend('Dual Echo', 'TE1', 'TE2'); % This is hard coded as TE1 or TE2, but what if you use a different TE? It is also much slower to constantly output figures in scripts.
        xlabel('Time points', 'FontSize', 14); ylabel('\DeltaR_2^{*} (1/s)', 'FontSize', 14);     %NS added symbols. 
    case 0
        h = figure; hold on;
        title('AIF \DeltaR_2^{*}');
        plot(mean(dR2s_TE1(final_loc,ss_tp:end)), 'k', 'LineWidth', 2);
        xlabel('Time points', 'FontSize', 14); ylabel('\DeltaR_2^{*} (1/s)', 'FontSize', 14);     
end
gca_set
savefig(h, 'AIF_dR2s_timecourse.fig');

%For visualization
figure('Visible', 'off'), hm = montage(permute(AIFmask, [1 2 4 3])); 
aif = hm.CData;  
figure('Visible', 'off'), hm = montage(permute(peak_frame, [1 2 4 3])); 
peak = hm.CData; close;
h2 = figure('Visible', 'on'); imagesc(imfuse(aif, peak)); 
axis off;
...
switch ConcFlag
    case 1
        switch echoFlag
            case 1
                disp('Calculating AIF in concentration');
...
switch echoFlag
    case 1
        ...
        switch ConcFlag
            case 1
                disp('AIF output in DR2s (first row) and concentration (second row)');
